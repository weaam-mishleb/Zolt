...
**נרמול ערים** ([`etl/cities.py`](etl/cities.py)): שופרסל שולח שם עיר (עם וריאנטים), ורמי לוי/אושר עד
שולחים **קוד למ"ס מספרי** (סמל יישוב). הנרמול ממיר קודים→שם, מאחד וריאנטים (`תל אביב-יפו`→`תל אביב`),
ומחלץ עיר מ-`storename` בעת הצורך — כך שכל הרשתות חולקות שם עיר קנוני אחד וההשוואה לפי עיר עובדת.
