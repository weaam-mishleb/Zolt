...
// Build { product_id: item } for quick cell lookup.
...
export default function ComparisonTable({ result }) {
  const { products, stores, winner_store_id, complete_store_count, store_count } = result
  ...
  return (
    <div className="mt-8 space-y-4">
      ...
      {winner ? (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-emerald-700">הסל הזול ביותר</p>
              <p className="font-bold text-emerald-900">
                {winner.chain_name} · {storeLabel(winner)}
              </p>
            </div>
          </div>
          <div className="text-2xl font-black text-emerald-700">{ils.format(winner.total)}</div>
        </div>
      ) : (
      ...
      <p className="text-sm text-slate-500">
        הושוו {store_count} סניפים · {complete_store_count} מחזיקים את כל המוצרים
      </p>

      {/* Products × stores matrix (horizontally scrollable, sticky product column) */}
          ...
          <tbody>
            {products.map((p) => (
              <tr key={p.id} className="border-t border-slate-100">
                <td className="sticky right-0 z-10 bg-white p-3 text-right font-medium text-slate-700">
                  {p.name || p.barcode}
                </td>
                {stores.map((s) => {
                  ...
                  if (!it || !it.found) {
                    return (
                      <td key={s.store_id} className="bg-amber-50/40 p-3 text-center">
                  ...
                  return (
                    <td
                      key={s.store_id}
                      className={`p-3 text-center text-slate-700 ${isWinner ? 'bg-emerald-50' : ''}`}
                    >
